 ...
 * Component
 * ------------------------- */
const AdvanceTaxInvoiceView = ({ invoiceData, heading }) => {
  ...
  /**
   * Use only the organization snapshot received in the selected advance-tax
   * invoice row. No organization API or Redux organization state is used here.
   */
  const seller = useMemo(() => {
    ...
    const branch =
      inv?.organizationBankBranch || inv?.bankBranch || inv?.branchName || "";
    const ifsc =
      inv?.organizationIfscCode || inv?.bankIfscCode || inv?.ifscCode || "";

    return {
      logoUrl: inv?.organizationLogoUrl || logo,
      ...
      // These remain empty unless the invoice row itself contains bank fields.
      bankName: inv?.organizationBankName || inv?.bankName || "",
      accountNo:
        inv?.organizationBankAccountNo ||
        inv?.bankAccountNo ||
        inv?.accountNo ||
        "",
  ...
  const grandTotal = toNumber(inv?.grandTotal ?? inv?.invoiceGrandTotal);

  // For an advance tax invoice, the complete invoice amount is due.
  const dueAmount = grandTotal;

  const hasBankDetails = Boolean(
    seller?.bankName || seller?.accountNo || seller?.branchIfsc,
  );
          ...
          <div className="border border-gray-300">
                ...
                <div className="grid grid-cols-2 border-b border-gray-300">
                  <div className="border-r border-gray-300 p-2.5">
                    ...
                    <div
                      ...
                      title={
                        inv?.irn || inv?.eInvoiceIrn || inv?.einvoiceIrn || ""
                      }
                    >
                      {inv?.irn || inv?.eInvoiceIrn || inv?.einvoiceIrn || "NA"}
                    </div>
                  </div>

                  <div className="p-2.5">
                    <div className="text-[10px] text-gray-500">
                      E-Invoice Acknowledgement Number
                    </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                <tr className="bg-gray-50">
                  <TableTd />
                  <TableTd className="text-right font-extrabold" colSpan={5}>
                    Due Amount
                  </TableTd>
                  <TableTd className="text-right font-extrabold">
                    {inrCurrency(dueAmount)}
                  </TableTd>
                </tr>
              </tbody>
            </table>
              ...
              <div className="text-[11px]">
                {hasBankDetails ? (
                  ...
                  <>
                    ...
                    <div>
                      PAN : <b>{seller.panNo || "-"}</b>
                    </div>
                    <div>
                      CIN : <b>{seller.cinNumber || "-"}</b>
                    </div>
                    <div>
...
export default memo(AdvanceTaxInvoiceView);
