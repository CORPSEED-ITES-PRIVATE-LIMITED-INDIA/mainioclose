...
export const getAllLeadUsers = createAsyncThunk("getAllUsers", async () => {
  const response = await api.get(`/leadService/api/v1/users/getAllUser`);
  return response.data;
});
...
export const createTicket = createAsyncThunk("createTicket", async (data) => {
  const response = await api.post(`/leadService/api/v1/createTicket`, data);
  return response.data;
});
...
export const LeadSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getAllLeads.fulfilled, (state, action) => {
        console.log('djkfgaskdjgkjsddshhsgsks',state.allLeads)
