 ...
 * ProformaInvoiceView
 * --------------------
 * Same paper layout/table structure/toolbar as TaxInvoice.jsx, but for the
 * estimate ("PI") API response (see getEstimateById), which nests buyer
 * details under company/unit and never carries an organizationXxx snapshot
 * the way a posted invoice does. TaxInvoice.jsx itself is shared by ~15
 * other real-invoice views that DO get flat organizationXxx/companyUnitXxx
 * fields, so this is a separate component rather than a change to it.
 */
 ...
 * Component
 * ------------------------- */
const ProformaInvoiceView = ({ invoiceData, heading }) => {
  ...
  // The estimate/PI response never carries an organization snapshot the way
  // a posted invoice does — fetch Corpseed's own org record for the seller
  // letterhead (GSTIN, PAN, address, bank details).
  useEffect(() => {
  ...
  const invoiceNumber = inv?.performanceInvoiceNumber || inv?.estimateNumber;
  const invoiceDate = inv?.estimateDate;

  // Seller (Corpseed) — falls back to the redux organization fetch since the
  // estimate payload itself has no organizationXxx fields.
  const seller = useMemo(() => {
    const addressParts = [
      inv?.organizationAddressLine1 || organizationDetail?.addressLine1,
      inv?.organizationAddressLine2,
      inv?.organizationCity || organizationDetail?.city,
      inv?.organizationState || organizationDetail?.state,
      inv?.organizationCountry || organizationDetail?.country,
    ].filter(Boolean);

    const address = addressParts.join(", ");
    const pinCode = inv?.organizationPinCode || organizationDetail?.pinCode;

    const gstin = inv?.organizationGstNo || organizationDetail?.gstNo || "";

    return {
      name: inv?.organizationName || organizationDetail?.name || "",
      addressLine1: `${address}${pinCode ? ` - ${pinCode}` : ""}`,
      gstin,
      stateName: inv?.organizationState || organizationDetail?.state || "",
      ...
      cinNumber:
        inv?.organizationCinNumber || organizationDetail?.cinNumber || "",
  ...
  // Buyer (bill to) is the client company; Consignee (ship to) is the
  // specific client unit — the estimate response nests these as separate
  // objects instead of the flat companyUnitXxx fields a posted invoice has.
  const billTo = useMemo(() => {
    const company = inv?.company || {};

    return {
      name: company?.name || inv?.companyName || "NA",
      gstin: inv?.unit?.gstNo || "",
      panNo: company?.panNo || "",
      address: [
        company?.city,
        company?.state,
  ...
  const shipTo = useMemo(() => {
    ...
    return {
      name: unit?.unitName || inv?.companyName || "NA",
      gstin: unit?.gstNo || "",
      address: [unit?.addressLine1, unit?.addressLine2, unit?.city]
        .filter(Boolean)
        .join(", "),
      stateName: unit?.state || "",
      stateCode: (unit?.gstNo || "").slice(0, 2),
  ...
  const handlePrint = () => {
    ...
    <html>
      <head>
        <title>${heading || "Proforma Invoice"} - ${invoiceNumber || ""}</title>
  ...
  const handleShareViaEmail = () => {
    const subject = encodeURIComponent(
      `${heading || "Proforma Invoice"} - ${invoiceNumber || ""}`,
    );
...
Corpseed Team`,
  ...
  const downloadPDF = async () => {
    ...
    pdf.save(`${invoiceNumber || "proforma-invoice"}.pdf`);
  };
          ...
          <div className="border border-gray-300">
            <div className="border-b border-gray-300 py-2 text-center">
              <div className="text-[16px] font-extrabold tracking-wide">
                {heading ? heading : "PROFORMA INVOICE"}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 border-b border-gray-300">
              ...
              <div className="p-2.5">
                ...
                {billTo.stateName ? (
                  <div className="text-[11px]">
                    State name : {billTo.stateName}
                  </div>
                ) : null}
                {billTo.panNo ? (
                  <div className="text-[11px]">PAN : {billTo.panNo}</div>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-gray-300 p-2.5">
              ...
              <div className="text-[11px]">
                ...
                <div>
                  Branch &amp; IFSC Code :{" "}
                  <b>
                    {seller.branchName || "NA"} & {seller.ifscCode || "NA"}
                  </b>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                >
                  <li>
                    This proforma invoice is for reference purposes only and
                    does not constitute a demand for payment until converted
                    into a final tax invoice.
                  </li>
                  <li>
                    Prices and taxes quoted are valid only until the date
                    mentioned above and are subject to change thereafter.
                  </li>
                  <li>
                    All statutory taxes, government fees, levies, and charges
                    applicable at the time of final invoicing shall be borne
                    entirely by the client.
                  </li>
                  <li>
                    Services shall commence only upon receipt of the agreed
                    payment/advance against this proforma invoice.
                  </li>
                </ul>
              </div>
            </div>
...
export default memo(ProformaInvoiceView);
