...
const INITIAL_VISIBLE_COLUMNS = ["id", "value", "actions"];
...
const CkEditorToken = () => {
  ...
  const handleEdit = (values) => {
    setFormData({ value: values?.value });
    setIsEdit(true);
    setItemId(values?.id);
    onOpen();
  };
  ...
  const handleSubmit = (values) => {
    if (isEdit) {
      dispatch(updateTokenInCkEditor({ id: itemId, ...values }))
        .then((resp) => {
          if (resp.meta.requestStatus === "fulfilled") {
            addToast({
      ...
      dispatch(addTokenInCkEditor(values))
        .then((resp) => {
          if (resp.meta.requestStatus === "fulfilled") {
            addToast({
  ...
  const renderCell = React.useCallback((rowData, columnKey) => {
    ...
    switch (columnKey) {
      case "value":
        return (
          <p className="text-bold text-small text-wrap">{rowData?.value}</p>
        );
      case "actions":
        return (
  ...
  return (
    <>
      <h1 className="font-sans text-2xl font-medium mb-1">License Keys list</h1>
      <Table
        isHeaderSticky
