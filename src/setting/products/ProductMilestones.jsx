...
// businessDaysEnabled, isAutoGenerated) is auto-populated in buildPayload
// with sensible defaults so the request still satisfies the DTO.
...
const ProductMilestones = ({ details }) => {
    ...
    (rowData, columnKey) => {
      ...
      switch (columnKey) {
        ...
        case "rollbackTatMinutes":
          return (
            <p className="text-[12.5px]">
              {rowData?.rollbackTatMinutes ?? 0} min
            </p>
          );
  ...
  // The DTO stores everything in MINUTES (see backend javadoc: "All TAT,
  // reminder and escalation values are stored in minutes"). The form already
  // collects minutes, so values are forwarded as-is — no unit conversion.
  ...
  const buildPayload = (values) => {
    ...
    return {
      ...
      reminderBeforeDueMinutes: toNumber(values.reminderBeforeDueMinutes),
      managerEscalationAfterDueMinutes: toNumber(
        values.managerEscalationAfterDueMinutes,
      ),
      hodEscalationAfterDueMinutes: toNumber(
            ...
            <>
              ...
              <ModalBody>
                ...
                >
                  ...
                  {/* TAT — collected in minutes and sent as-is to the API */}
