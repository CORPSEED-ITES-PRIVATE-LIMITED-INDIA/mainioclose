...
const columns = [
  ...
  { name: "UNIT NAME", uid: "companyName", sortable: true },
...
const CompanyUnits = () => {
  const { userId, companyId, stateName } = useParams();
  const dispatch = useDispatch();
  const count = useSelector((state) => state.company.companyUnitList?.length);
  ...
  useEffect(() => {
    ...
    dispatch(getCompanyUnitsByStateAndCompanyId({ companyId, stateName }));
    dispatch(getAllContactListById(companyId));
  }, [dispatch, companyId, stateName]);
